...
Every released version is tagged in git as `v<version>`, so any previous
version can be restored with `git checkout v<version>` (or by asking Claude
to roll back to it). The same history is shown in-app under
**Settings → Version**. Keep `js/version.js` in step with this file.
